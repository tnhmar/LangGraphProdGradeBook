...
is the question of what happens below that contract: who decides which model to
call, what happens when the call fails at the network or provider level, and who
pays attention to the bill.
...
This chapter builds the full LLM infrastructure layer. You will configure a
LiteLLM router that abstracts all provider calls behind a single interface while
pinning exact model versions. You will add Tenacity decorators that retry on
retriable errors and pass immediately on non-retriable ones. You will implement
a circuit breaker from first principles so you understand the three-state
machine rather than hiding it behind a library call. You will wire LiteLLM's
callback system to an \code{AgentState} budget field that tracks cost in real
time, and you will implement a budget circuit breaker that stops the graph
cleanly when the budget is exhausted. You will build a fallback cascade that
logs every degradation event to Langfuse. Finally, you will externalise every
model name and configuration value into Pydantic Settings so that no hardcoded
string ever appears in application code.
...
graph-edge outcomes. Table~\ref{tab:exception-routing} summarises the mapping
before you build it, so you can keep the whole picture in mind as each piece
appears.
...
%% ------------------------------------------------------------
\section{LiteLLM Router: Provider Fallback and Model Version Pinning}
...
site to a single provider. If that provider degrades, the call fails and there is
no recovery path. Second, the model string \code{"gpt-4o"} is an alias that
...
%% ------------------------------------------------------------
\section{Tenacity Decorators: Per-Exception Retry Strategies}
\label{sec:tenacity}
%% ------------------------------------------------------------
...
%% ------------------------------------------------------------
\section{Circuit Breaker: A Three-State Machine}
...
class LLMCircuitBreaker:
    ...
    def call(
        ...
        **kwargs: Any,
        ...
        # CLOSED or HALF_OPEN: attempt the call outside the lock so
        # we do not block other threads during a potentially slow I/O.
        try:
    ...
    def _record_success(self) -> None:
        with self._lock:
            if self.state == CircuitState.HALF_OPEN:
                # Probe succeeded: fully close and reset timeout
                self._failure_count   = 0
...
%% ------------------------------------------------------------
\section{Cost Tracking: Callback to AgentState Budget}
\label{sec:cost-tracking}
%% ------------------------------------------------------------

LiteLLM emits a \code{success\_callback} after every completed call. The
callback receives the full response object including token usage. This is the
correct place to accumulate cost: it fires exactly once per successful call,
after the response has been validated, and it has access to
\code{litellm.completion\_cost()} which understands per-model pricing.

The \code{CostTracker} class registers a method as the callback and accumulates
...
class CostTracker:
    ...
    def __init__(self, budget: TokenBudget, router: litellm.Router) -> None:
        self._budget = budget
        self._lock   = threading.Lock()
        # Register on the Router instance, not on litellm globally
...
%% ------------------------------------------------------------
\section{Budget Circuit Breaker: Graceful Stop on Overage}
...
%% ------------------------------------------------------------
\section{Fallback Routing Cascade: Three Tiers, Full Observability}
...
\code{FallbackTier} object with its own circuit breaker, and every attempt---
successful or not---is logged as a Langfuse event with a consistent schema.
...
class FallbackTier:
    ...
    cost_ceiling_usd: float   # skip tier if remaining budget < this value
...
class FallbackRouter:
    ...
    def __init__(
        ...
        tracker:  CostTracker,
        langfuse: Langfuse,
        router,           # litellm.Router from RouterFactory
    ) -> None:
        self._tiers    = tiers
    ...
    def route(
        ...
        for tier_index, tier in enumerate(self._tiers):
            ...
            try:
                ...
                call_cost = float(
                    getattr(response, "_hidden_params", {}).get("response_cost", 0.0)
                )
                self._log_attempt(
                    tier=tier, tier_index=tier_index,
                ...
                # Update AgentState fields for downstream nodes and tests
        ...
        # All tiers exhausted: return static fallback
        self._log_attempt(
...
%% ------------------------------------------------------------
\section{RunnableConfig and Pydantic Settings: Zero Hardcodes}
\label{sec:settings}
%% ------------------------------------------------------------
...
class LLMSettings(BaseSettings):
    ...
    primary_model:          str       # LLM_PRIMARY_MODEL
    fallback_model_1:       str       # LLM_FALLBACK_MODEL_1
    fallback_model_2:       str       # LLM_FALLBACK_MODEL_2
    openai_api_key:         SecretStr # LLM_OPENAI_API_KEY
    anthropic_api_key:      SecretStr # LLM_ANTHROPIC_API_KEY
    total_budget_usd:       float     # LLM_TOTAL_BUDGET_USD
    failure_threshold:      int   = 5
    ...
    def validate_model_pin(cls, v: str) -> str:
        ...
        if not _MODEL_PIN_RE.match(v):
            raise ValueError(
                f"Model name must be in 'provider/model-version' format, got: {v!r}. "
                f"Using bare aliases risks silent version drift."
            )
        return v

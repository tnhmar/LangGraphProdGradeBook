...
\code{response_model=} argument. Under the hood, it can use three strategies to
...
object into \code{last_action}. Validation lived inline inside the node. That is
...
\begin{itemize}
  \item \code{_raw_model_output: str | None} --- the last raw JSON string the
        model produced,
  \item \code{_validation_error: str | None} --- the last Pydantic
        \code{ValidationError} string, and
  \item \code{_validation_attempt: int} --- the number of consecutive failed
...
\code{last_action} as a fully parsed union and clears the error fields. On
failure it increments \code{_validation_attempt}, records the error, and leaves
\code{last_action} untouched.
...
for \code{max_cycles}, \code{token_ceiling}, \code{wall_clock_timeout}, and
\code{cost_cap}. Here you add a new field \code{validation_max_retries} with a
...
When validation has failed \code{validation_max_retries} times in a row for the
...
update. It writes a structured summary into \code{final_answer}, sets
\code{reflect_decision} to \code{"ESCALATE"}, and leaves the error fields
...
\code{route_after_validation} has returned \code{"escalate"}. The Reflect node
then sees \code{reflect_decision == "ESCALATE"} and Chapter~1's
\code{route_from_reflect} function routes directly to \code{END}. The
\code{termination_status} in the \code{CycleTrace} is set to \code{"FAILURE"} by
...
Studio can filter observations where \code{event == "validation_failure"} and
...
\begin{notebox}
As in Chapter~1's tests, set \code{LANGFUSE_ENABLED=false} in your test
...
On the graph side, Reason writes \code{_raw_model_output} instead of a parsed
\code{last_action}. Validation consumes that field and writes \code{last_action}
...
constructs a minimal \code{AgentState}, populates \code{_raw_model_output}, and
